...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if(mappages(d, (void*)i, PGSIZE, PADDR(mem), writable|PTE_U) < 0)
      goto bad;
  }
  return d;
...
{
  pte_t *pte;
  if ((unsigned long)addr % PGSIZE != 0 || (uint)addr + len * PGSIZE > USERTOP || len <= 0){
    return  -1;
  }
  for (void* i = addr; i < addr + len * PGSIZE; i = i + PGSIZE){
    pte = walkpgdir(proc->pgdir, i, 0);
    if (pte != 0){
      *pte = *pte & ~PTE_W;
      lcr3(PADDR(proc->pgdir));
    }
    else
      return -1; // does not find pte, fail to change protect bit
